...
func TestPlanRejectsNonCanonicalMinio(t *testing.T) {
	...
	_, err = makePlan(options{suite: "real", modes: join(realModes), workloads: join(realWorkloads), profile: "4-node", topology: topology, minio: "http://minio", result: t.TempDir(), repetitions: 1, latency: 20, memory: 20, warm: 5})
	if err == nil {
...
func TestRemoteArgsPropagateSampling(t *testing.T) {
	...
	for _, required := range []string{
		...
		"--latency-iterations 20", "--memory-iterations 20", "--warm-invocations 5",
	} {
