...
class CalibrationTest(unittest.TestCase):
    ...
    def test_runner_uses_archived_worker_config_not_the_checked_in_default(self):
        ...
        self.assertIn('tree=flamegraph', source)
        self.assertIn('FLAMEGRAPH_COMMIT', source)
        self.assertIn('experiment/e2/validate_evidence.py', source)
        self.assertIn('evidence_validator_sha256=', source)
        self.assertIn('evidence_status=$evidence_status', source)
        self.assertIn('perf_artifact_count=$perf_artifact_count', source)
        self.assertIn('scripts/util/wait_prometheus_ready.sh', source)
