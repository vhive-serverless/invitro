...
func distributeLoaderSSHKey(loaderNode string, allNodes []string) error {
	_, err := loaderUtils.ServerExec(loaderNode, `echo -e "\n\n\n" | ssh-keygen -t rsa -N ""`)
	if !utils.CheckErrorWithMsg(err, "Failed to generate SSH key on loader node %s: %v\n", loaderNode, err) {
		return err
	}

	pubKey, err := loaderUtils.ServerExec(loaderNode, "cat ~/.ssh/id_rsa.pub")
